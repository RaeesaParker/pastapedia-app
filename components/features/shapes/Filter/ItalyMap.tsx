...
// Map paths for each region group
  ...
  return (
    <View style={styles.container}>
      <Svg width="100%" height="100%" viewBox="0 0 500 650">
        <G>
          {(Object.keys(REGION_GROUPS) as RegionGroupName[]).map(
            (groupName) => {
              const groupData = REGION_GROUPS[groupName];
              const pathData = REGION_PATHS[groupName];
              const isSelected = isRegionGroupSelected(groupData.regions);
              ...
              return (
                <G key={groupName}>
                  <Path
                    ...
                    strokeWidth={2}
                    onPress={() => {
                      console.log("The label for", groupName, "was pressed");
                      console.log("Data is:", groupData);
                      handleRegionPress(groupData.regions);
                    }}
                  />
                  <SvgText
                    x={pathData.label.x}
